...
interface inviteMentorProps
{
 ...
 export const inviteMentore = async (body:inviteMentorProps) => {
    const response=  apiCall({"url":endPoints.adminMentors,'method':"POST", "data":body,'tag':'invite-mentor'});
   ...
   export const mentorDelete=async(url:string)=>{
    const response=  apiCall({"url":url,'method':"DELETE",'tag':'delete-mentor'});
      const data = await response
      return data; 
   }
