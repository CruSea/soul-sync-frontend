...
import { SignInButton, UserButton, SignedIn, SignedOut, useSession, useUser } from "@clerk/nextjs";
import { useEffect } from 'react';

const UserActions = () => {
    // Using useSession to get the session data
    const { session } = useSession();

    // Using useUser to get user data
    const { user, isLoaded, isSignedIn } = useUser();

    useEffect(() => {
      // Log the sessionId or handle session info once it's available
      if (session) {
    ...
    useEffect(() => {
      // Log the user email or session ID when user data is available
      if (isLoaded && user) {
  ...
  return (
    <div className="flex gap-6 items-center">
      ...
      <svg className="cursor-pointer" width="30" height="30" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path fillRule="evenodd" clipRule="evenodd" d="M7.25 11C7.25 10.5858 7.58579 10.25 8 10.25H16C16.4142 10.25 16.75 10.5858 16.75 11C16.75 11.4142 16.4142 11.75 16 11.75H8C7.58579 11.75 7.25 11.4142 7.25 11Z" fill="#111827" />
        <path fillRule="evenodd" clipRule="evenodd" d="M7.25 15C7.25 14.5858 7.58579 14.25 8 14.25H14C14.4142 14.25 14.75 14.5858 14.75 15C14.75 15.4142 14.4142 15.75 14 15.75H8C7.58579 15.75 7.25 15.4142 7.25 15Z" fill="#111827" />
        <rect x="15" y="1" width="10" height="10" rx="5" fill="#E03137" />
        <rect x="15" y="1" width="10" height="10" rx="5" stroke="white" strokeWidth="2" />
      </svg>

      {/* when not signed in it will show a sign in button*/}
      <SignedOut>
      ...
      {/* when signed in it will show the user account*/}
      <SignedIn>
