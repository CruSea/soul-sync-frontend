...
export const getStartedMentorFormSchema = z.object({
  ...
  expertise: z
    .array(
      ...
      {
        required_error: 'You need to select at least one expertise',
      }
    )
    .min(1, 'You need to select at least one expertise'),
  ...
  capacity: z.preprocess(
    ...
    z
      ...
      .max(20, { message: 'Capacity must be no more than 20.' })
  ),
...
export const getStartedAdminFormSchema = z.object({
  ...
  phoneNumber: z.string().refine(
    (value) => {
      if (value.startsWith('+')) {
        return value.length === 13 && /^\+\d{12}$/.test(value);
      } else {
...
// Update interface props to use expertise instead of specialization
export interface AgeFieldProps {
  control: any;
  className?: string;
}

export interface ExpertiseFieldProps {
  control: any;
